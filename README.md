...
2. **Parameters**: Number of tables, seats per table, rounds, attempts, and worker count can be set via environment variables.
...
6. **Output**: Writes the best seating arrangement to `seatings.txt` (annotated with each person's attributes) and prints the number of unique connections along with the total weighted value.
...
```bash
TABLES=10 SEATS=5 ROUNDS=3 ATTEMPTS=500 go run .
```

### 3. View the Output

The program generates a `seatings.txt` file with the seating arrangements for each round and table, annotated with department, role, and country. It also prints the number of unique connections and the total weighted value achieved.
